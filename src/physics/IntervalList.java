...
public class IntervalList implements Serializable {
	...
	/**
	 * An Interval is an immutable representation of a single contigous range of
	...
	public static class Interval implements Comparable, Serializable {
		...
		private Interval restrictLength(double length) {
			if (!Double.isInfinite(start) && !Double.isInfinite(end) && (end - start > length)) {
		...
		private Interval restrictTo(Interval i) {
			...
			return new Interval(start > i.start ? start : i.start, end < i.end ? end : i.end);
		}
		...
		public boolean equals(Object o) {
			if (o instanceof Interval) {
				...
				if (this.start == i.start && this.end == i.end) {
					return true;
				} else {
					return false;
				}
	...
	/**
	 * @effects: creates a new IntervalList which contains a copy of the ranges
	 *           represented by <code>il</code>.
	 **/
	...
	/**
	 * @effects: returns true iff <code>this</code> represents the empty set of
	 *           intervals.
	 **/
	 ...
	 * @requires: end >= start
	 *
	 * @effects: adds the range from <code>start</code> to <code>end</code> to
	 *           this.
	 **/
	 ...
	 * @requires: end >= start
	 *
	 * @effects: removes from this all of the numbers which are not between
	...
	/**
	 * @effects: removes from this all of the numbers which are not contained in
	 *           the range represented by <code>i</code>.
	 **/
	...
	/**
	 * @effects: shrinks the size of each continous range of numbers contained
	 *           in this to only be of <code>length</code>. Each continous range
	 *           will be aligned to have the same start it originally had, but
	 *           the end point will be no farther than <code>length</code> away.
	 **/
	...
	/**
	 * @effects: returns an Iterator which will return, in increasing order,
	 *           Intervals representing the numbers contained in this.
	 **/

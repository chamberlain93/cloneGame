 ...
 *
 * <p>
 * The library is described in terms of these concepts: <br>
 * <ul>
 ...
 * <p>
 * <ol>
 * <li>The client calls the timeUntilCollision() methods to calculate the times
 * at which the ball(s) will collide with each of the bouncers or with another
 * ball. The minimum of all these times (call it "mintime") is the time of the
 * next collision.
 *
 * <li>The client updates the position of the ball(s) and the bouncers to
 * account for mintime passing. At this point, the ball and the object it is
 * about to hit are exactly adjacent to one another.
 *
 * <li>The client calls the appropriate reflect() method to calculate the change
 ...
 * accounted for, the client must do so before or after the of the "time until /
 * update position / reflect" series of steps - never inbetween those three
 * steps.
 * 
 * <p>
 * <a name="endpoint_effects"></a>
 *
 * <b>Important note</b>: The methods which deal with line segment bouncers do
 * NOT deal with the end-points of the segment. To ensure realistic behavior,
 * shapes should be constructed from a combination of line segments with
...
public class Geometry {
	 ...
	 *
	 * @effects changes which implementation of <code>GeometryInterface</code>
	 *          will be used to service the static methods of this class. Most
	 *          users will prefer to use <code>setForesight</code> or
	 *          <code>setTuningParameters</code> instead.
	 *
	 * @see #setForesight
	 ...
	 * <code>useDoughnut</code> is true then doughnut optimizations are enabled.
	 * The values used by default are &lt;+Inf, true, 15&gt;. Many uses may
	 * prefer to simply use <code>setForesight</code> instead.
	 *
	 ...
	 *            farther than <code>maximumForesight</code> in the future, but
	 *            no extra effort will be made to find them. If set to
	 ...
	 *            searched for. Collisions may still be returned that happen
	 *            farther than <code>maximumForesight</code> in the future, but
	 *            no extra effort will be made to find them.
	 *
	 * @see Double#POSITIVE_INFINITY
	 **/
	...
	/**
	 * <code>DoublePair</code> is a simple immutable record type representing a
	 * pair of <code>double</code>s.
	 **/
	public static class DoublePair implements Serializable {
		...
		/**
		 * Creates a DoublePair with <code>d1</code> and <code>d2</code> as
		 * given
		 **/
		...
		/**
		 * Creates a DoublePair with <code>d1</code> and <code>d2</code> both
		 * set to the given argumen
		 **/
		...
		public boolean areFinite() {
			return !Double.isInfinite(d1) && !Double.isInfinite(d2) && !Double.isNaN(d1) && !Double.isNaN(d2);
		}
	...
	/**
	 * <code>VectPair</code> is a simple immutable record type representing a
	 * pair of <code>Vect</code>s.
	 * 
	 * @see Vect
	 **/
	public static class VectPair implements Serializable {
		...
		/**
		 * Creates a VectPair with <code>v1</code> and <code>v2</code> as given
		 **/
		...
		public int hashCode() {
			return ((v1 == null) ? 0 : (3 * v1.hashCode())) + ((v2 == null) ? 0 : (7 * v2.hashCode()));
		}
	}

	/**
	 * DoublePair with both <code>d1</code> and <code>d2</code> set to
	 ...
	 *         by a*x<sup>2</sup> + b*x + c = 0, or <code>NaN</code> if no real
	 *         roots exist.
	 *
	 * @see java.lang.Double#NaN
	 **/
	 ...
	 * <code>line</code> which forms a line with <code>point</code> that is
	 * perpendicular to <code>line</code>.
	 *
	 * @requires <code>line</code> has non-zero length
	 *
	 ...
	 *
	 * @effects given an initial velocity, <code>incidentVect</code>, and the
	 ...
	 * @requires <code>line</code> has non-zero length
	 *
	 * @effects computes the time until a circular ball travelling at a
	 *          specified velocity collides with a specified line segment. If no
	 *          collision will occur, <tt>POSITIVE_INFINITY</tt> is returned.
	 *          This method assumes that the ball will travel with constant
	 ...
	 *
	 * @effects computes the new velocity of a ball reflecting off of a wall.
	 *          The velocity resulting from this method corresponds to collision
	 *          against a surface with the given reflection coefficient. A
	 *          reflection coefficient of 1 indiciates a perfectly elastic
	 *          collision. This method assumes that the ball is at the point of
	 *          impact.
	 *
	 * @param line
	 ...
	 *
	 * @effects computes the new velocity of a ball reflecting off of a wall.
	 *          The velocity resulting from this method corresponds to a
	...
	/**
	 * @return the square of the distance between two points represented by
	 ...
	 *
	 * @effects computes the new velocity of a ball reflecting off of a circle.
	 *          The velocity resulting from this method corresponds to a
	...
	/**
	 * Rotates the point represented by <code>p</code> by <code>a</code> around
	 * the center of rotation, <code>cor</code>, and returns the result.
	 *
	 ...
	 *
	 * @param a
	 *            the amount by which to rotate <code>point</code>
	 *
	 * @return point <code>point</code> rotated around <code>cor</code> by
	 *         <code>a</code>
	 **/
	...
	/**
	 * Rotates the line segment represented by <code>line</code> by
	 * <code>a</code> around the center of rotation, <code>cor</code>, and
	 * returns the result.
	 *
	 * @effects rotates the line segment represented by <code>line</code> by
	 *          <code>a</code> around the center of rotation, <code>cor</code>,
	 *          and returns the result.
	 *
	 * @param line
	 ...
	 *            the amount by which to rotate <code>point</code>
	 *
	 * @return line segment <code>line</code> rotated around <code>cor</code> by
	 *         <code>a</code>
	 **/
	...
	/**
	 * Rotates the circle represented by <code>circle</code> by <code>a</code>
	 * around the center of rotation, <code>cor</code>, and returns the result.
	 *
	 * @effects rotates the circle represented by <code>circle</code> by
	 *          <code>a</code> around the center of rotation, <code>cor</code>,
	 *          and returns the result.
	 *
	 * @param circle
	 ...
	 *            the amount by which to rotate <code>point</code>
	 *
	 * @return circle <code>circle</code> rotated around <code>cor</code> by
	 *         <code>a</code>
	 **/
	 ...
	 *
	 * @return the times until intersection, with lesser result in d1, or
	 *         <tt>+Inf</tt>s if no collisions will occur
	 *
	 * @see Double#POSITIVE_INFINITY
	 **/
	 ...
	 *
	 * @effects computes the time until a circular ball travelling at a
	 *          specified velocity collides with a specified line segment which
	 *          is rotating at a fixed angular velocity about a fixed center of
	 *          rotation.
	 *
	 *          <p>
	 * 			<img src="doc-files/rotate_line.gif">
	 *
	 * @param line
	 *            the initial position of the rotating line segment (wall)
	 *
	 * @param center
	 *            the center of rotation for <code>line</code>
	 *
	 * @param angularVelocity
	 *            the angular velocity of the rotation of <code>line</code> in
	 ...
	 * @see <a href="#endpoint_effects">endpoint effects</a>
	 **/
	public static double timeUntilRotatingWallCollision(LineSegment line, Vect center, double angularVelocity,
	 ...
	 *
	 * @requires <code>line</code> has non-zero length && the ball is at the
	 *           point of impact
	 *
	 * @effects computes the new velocity of a ball reflecting off of a wall
	 *          which is rotating about a point with constant angular velocity.
	 *          The velocity resulting from this method corresponds to a
	 *          perfectly elastic collision. This method assumes that the ball
	 ...
	 *
	 * @param center
	 *            the point about which <code>line</code> rotates
	 *
	 * @param angularVelocity
	 *            the angular velocity at which <code>line</code> rotates, in
	 *            radians per second. A positive angular velocity denotes a
	 *            rotation in the direction from the positive x-axis to the
	 *            positive y-axis.
	 *
	 ...
	 **/
	public static Vect reflectRotatingWall(LineSegment line, Vect center, double angularVelocity, Circle ball,
			Vect velocity) {
	 ...
	 *
	 * @requires <code>line</code> has non-zero length && the ball is at the
	 *           point of impact && <code>reflectionCoeff</code> >= 0
	 *
	 * @effects computes the new velocity of a ball reflecting off of a wall
	 *          which is rotating about a point with constant angular velocity.
	 *          The velocity resulting from this method corresponds to a
	 *          collision against a surface of the given reflection coefficient.
	 *          A reflection coefficient of 1 indicates a perfectly elastic
	 *          collision. This method assumes that the ball is at the point of
	 *          impact. If the ball does not hit in between the endpoints of
	 *          <code>line</code>, <code>velocity</code> is returned.
	 *
	 * @param line
	 *            the line segment representing the initial position of the
	 *            rotating wall
	 ...
	 **/
	public static Vect reflectRotatingWall(LineSegment line, Vect center, double angularVelocity, Circle ball,
			Vect velocity, double reflectionCoeff) {
	 ...
	 *
	 * @effects computes the time until a circular ball travelling at a
	 *          specified velocity collides with a specified circle that is
	 *          rotating about a given center of rotation at a given angular
	 *          velocity. If no collision will occurr <tt>POSITIVE_INFINITY</tt>
	 *          is returned. This method assumes the ball will travel with
	 ...
	 *
	 * @param angularVelocity
	 *            the angular velocity with which <code>circle</code> is
	 *            rotating about <code>center</code>, in radians per second. A
	 *            positive angular velocity denotes a rotation in the direction
	...
	public static double timeUntilRotatingCircleCollision(Circle circle, Vect center, double angularVelocity,
			Circle ball, Vect velocity) {
	 ...
	 *
	 * @effects computes the new velocity of a ball reflected off of a circle
	 *          which is rotating with constant angular velocity around a point.
	 *          The velocity resulting from this method corresponds to a
	 ...
	 *
	 * @param center
	 *            the point about which <code>circle</code> is rotating
	 *
	 * @param angularVelocity
	 *            the angular velocity with which <code>circle</code> is
	 *            rotating about <code>center</code>, in radians per second. A
	 ...
	 **/
	public static Vect reflectRotatingCircle(Circle circle, Vect center, double angularVelocity, Circle ball,
			Vect velocity) {
	...
	public static Vect reflectRotatingCircle(Circle circle, Vect center, double angularVelocity, Circle ball,
			Vect velocity, double reflectionCoeff) {
		return geometry.reflectRotatingCircle(circle, center, angularVelocity, ball, velocity, reflectionCoeff);
	}
	 ...
	 *
	 * @effects computes the time until two balls, represented by two circles,
	 *          travelling at specified constant velocities, collide. If no
	 *          collision will occur <tt>POSITIVE_INFINITY</tt> is returned.
	 ...
	 *
	 * @requires mass1 > 0 && mass2 > 0 && the distance between the two balls is
	 *           approximately equal to the sum of their radii; that is, the
	 *           balls are positioned at the point of impact.
	 *
	 * @effects computes the resulting velocities of two balls which collide.
	 *
	 * @param center1
	 ...
	 *            the velocity of the second ball before impact
	 *
	 * @return a <code>VectPair</code>, where the first <code>Vect</code> is the
	 *         velocity of the first ball after the collision and the second
	 *         <code>Vect</code> is the velocity of the second ball after the
	 *         collision.
	 **/
	public static VectPair reflectBalls(Vect center1, double mass1, Vect velocity1, Vect center2, double mass2,
			Vect velocity2) {

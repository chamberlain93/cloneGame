 ...
 * two other concrete implementations and checks that they both have the same
 * behavior.
 *
...
public class GeometryCompare implements GeometryInterface {
	...
	/**
	 * The tolerance which is used during comparison; results may differ by this
	 * value at most.
	 **/
	 ...
	 *
	 * @effects returns a new GeometryCompare which dispatches to both g1 and
	 *          g2, throws an exception if the answers differ, and returns the
	 *          result from g1.
	 **/
	public GeometryCompare(GeometryInterface g1, GeometryInterface g2) {
		if (g1 == null)
			throw new IllegalArgumentException("g1 null");
		if (g2 == null)
			throw new IllegalArgumentException("g2 null");
		this.g1 = g1;
		this.g2 = g2;
	}
	...
	/**
	 * A string thunk is a way of delaying the (expensive) creation of Strings
	 * when they won't usually be used, anyway. For example, the failure message
	 * for an assertion is rarely used, so should only be computed when the
	 * assertion fails. (See section 4.2.2 in SICP by Abelson, et. al.)
	 **/
	...
	public DoublePair quadraticSolution(double a, double b, double c) {
		return compareAndReturn(NONE, g1.quadraticSolution(a, b, c), g2.quadraticSolution(a, b, c));
	}

	// specification taken from GeometryInferface; not strengthened
	public double minQuadraticSolution(double a, double b, double c) {
		return compareAndReturn(NONE, g1.minQuadraticSolution(a, b, c), g2.minQuadraticSolution(a, b, c));
	}

	// specification taken from GeometryInferface; not strengthened
	public Vect perpendicularPoint(LineSegment line, Vect point) {
		return compareAndReturn(NONE, g1.perpendicularPoint(line, point), g2.perpendicularPoint(line, point));
	}
	...
	public Vect reflectWall(LineSegment line, Vect velocity) {
		return compareAndReturn(NONE, g1.reflectWall(line, velocity), g2.reflectWall(line, velocity));
	}

	// specification taken from GeometryInferface; not strengthened
	public double distanceSquared(Vect v1, Vect v2) {
		return compareAndReturn(NONE, g1.distanceSquared(v1, v2), g2.distanceSquared(v1, v2));
	}

	// specification taken from GeometryInferface; not strengthened
	public double distanceSquared(double x1, double y1, double x2, double y2) {
		return compareAndReturn(NONE, g1.distanceSquared(x1, y1, x2, y2), g2.distanceSquared(x1, y1, x2, y2));
	}
	...
	public Vect rotateAround(Vect point, Vect cor, Angle a) {
		return compareAndReturn(NONE, g1.rotateAround(point, cor, a), g2.rotateAround(point, cor, a));
	}

	// specification taken from GeometryInferface; not strengthened
	public LineSegment rotateAround(LineSegment line, Vect cor, Angle a) {
		return compareAndReturn(NONE, g1.rotateAround(line, cor, a), g2.rotateAround(line, cor, a));
	}

	// specification taken from GeometryInferface; not strengthened
	public Circle rotateAround(Circle circle, Vect cor, Angle a) {
		return compareAndReturn(NONE, g1.rotateAround(circle, cor, a), g2.rotateAround(circle, cor, a));
	}
	...
	// specification taken from GeometryInferface; not strengthened
	public double timeUntilRotatingWallCollision(final LineSegment line, final Vect center,
			final double angularVelocity, final Circle ball, final Vect velocity) {
		StringThunk message = new StringThunk() {
			public String string() {
				return "timeUntilRotatingWallCollision(" + line + " " + center + " " + angularVelocity + " " + ball
						+ " " + velocity + ")";
			}
		};
	...
	public Vect reflectRotatingWall(LineSegment line, Vect center, double angularVelocity, Circle ball, Vect velocity,
		...
		return compareAndReturn(NONE,
				g1.reflectRotatingWall(line, center, angularVelocity, ball, velocity, reflectionCoeff),
	...
	public double timeUntilRotatingCircleCollision(Circle circle, Vect center, double angularVelocity, Circle ball,
			Vect velocity) {
		return compareAndReturn(NONE,
				g1.timeUntilRotatingCircleCollision(circle, center, angularVelocity, ball, velocity),
	...
	public Vect reflectRotatingCircle(Circle circle, Vect center, double angularVelocity, Circle ball, Vect velocity,
			double reflectionCoeff) {
		return compareAndReturn(NONE,
				g1.reflectRotatingCircle(circle, center, angularVelocity, ball, velocity, reflectionCoeff),
	 ...
	 *
	 * @throws a
	 *             RuntimeException whose message includes information about the
	 *             two values, their difference, and the provided message
	 **/
	private static void error(StringThunk message, double a, double b) {
		throw new RuntimeException(
				"Assertion error: " + a + " != " + b + "; diff = " + (a - b) + "\n" + message.string());
	}

	/**
	 * @effects does nothing if a and b are equal within the given tolerance
	 * @throws RuntimeException
	 *             if the values are not within tolerance
	 * @see #TOLERANCE
	 **/
	...
	/**
	 * @effects returns a if a and b are within tolerance of each other, else
	...
	/**
	 * @effects returns a if the component of a and b are within tolerance of
	 *          each other, else throws a RuntimeException
	 **/

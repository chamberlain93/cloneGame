...
/**
 * GeometryImpl is the default implementation of GeometryInterface. Most callers
 * will not use GeometryImpl directly, but will use the singleton Geometry
 * instead.
 *
 * <p>
 ...
 * <li>
 *
 * <code>maximumForesight</code> indicates the maximal distance in the future
 * that the client is interested in learning about collisions. Collisions past
 * this time might still be reported, but are not guaranteed to be found. In
 * short, this parameter causes the implementation to focus on finding
 * collisions within this time at the possible expense of not detecting
 * collisions past this time. This default value is +infinity.
 *
 * </li>
 * <p>
 * <li>
 *
 * <code>searchSlices</code> determines the number of slices used to search for
 * roots when using iterative numerical root finding techniques. The default
 * value is 15. Setting a higher value will increase the chance of finding
 * collision in the timeUntilRotating* methods, but will cause them to run
 * slower.
 *
 * </li>
 * </ul>
...
public class GeometryImpl implements GeometryInterface {
	...
	/**
	 * @effects Constructs a GeometryImpl with the default tuning parameters as
	 ...
	 *
	 * @effects Constructs a GeometryImpl with the specified tuning parameters
	...
	public Vect perpendicularPoint(LineSegment line, Vect point) {
		double x1 = line.p1().x(), y1 = line.p1().y(), x2 = line.p2().x(), y2 = line.p2().y(), a = point.x(),
				b = point.y();
	...
	public double timeUntilWallCollision(LineSegment line, Circle ball, Vect velocity) {
		double x1 = line.p1().x(), y1 = line.p1().y(), x2 = line.p2().x(), y2 = line.p2().y(), a = ball.getCenter().x(),
				b = ball.getCenter().y(), radius = ball.getRadius(), va = velocity.x(), vb = velocity.y();
		...
		double minS = ((width * (cX - x1)) + (height * (cY - y1))) / ((width * width) + (height * height));
	...
	public double timeUntilCircleCollision(Circle circle, Circle ball, Vect velocity) {
		Vect xy = circle.getCenter(), ab = ball.getCenter();
		double x = xy.x(), y = xy.y(), radius1 = circle.getRadius(), a = ab.x(), b = ab.y(), radius2 = ball.getRadius(),
				va = velocity.x(), vb = velocity.y();
	...
	public Vect reflectCircle(Vect circle, Vect ball, Vect velocity, double reflectionCoeff) {
		return applyReflectionCoeff(velocity, reflectCircle(circle, ball, velocity), reflectionCoeff);
	}
	...
	public LineSegment rotateAround(LineSegment line, Vect cor, Angle a) {
		return new LineSegment(rotateAround(line.p1(), cor, a), rotateAround(line.p2(), cor, a));
	}
	...
	public Circle rotateAround(Circle circle, Vect cor, Angle a) {
		return new Circle(rotateAround(circle.getCenter(), cor, a), circle.getRadius());
	}
	...
	/**
	 * Restricts the ranges of times in <code>intervals</code> to only contain
	 * periods of time when two pie slices (one rotating) overlap.
	 *
	 * @requires: alpha, beta, and phi to be in the range -2pi to 2pi && width
	 *            to be in the range 0 to 2pi
	 *
	 * @modifies: intervals
	 *
	 * @effects: Let the stationary arc consist of the arc running from alpha to
	 *           beta. If bigSlice is true the arc should be such that it
	 *           subsumes more than pi radians, if bigSlice is false the arc
	 *           should be such that it subsumes less than pi radians. Also let
	 *           the rotating arc start at phi and extend width radians in the
	 *           counter clockwise direction. Let the rotating arc be rotating
	 *           at omega radians per unit of time in the counter clockwise
	 *           direction and be located at the location described above at
	 *           time 0. This function will restrict the contents of intervals
	 *           to only contain periods of time during which the two arcs
	 *           overlap. This method only operates and effects periods of time
	 *           in the interval from 0 to 4pi.
	 **/
	private void restrictIntervalByAngle(IntervalList intervals, double alpha, double beta, double omega, double phi,
			double width, boolean bigSlice) {
		if (alpha < 0)
			alpha += 2 * Math.PI;
		if (beta < 0)
			beta += 2 * Math.PI;
		...
		if ((d < Math.PI && bigSlice) || (d > Math.PI && !bigSlice)) {
			double t = alpha;
			alpha = beta;
			beta = t;
		}
	 ...
	 *
	 * @effects: Let the stationary arc consist of the arc running from alpha to
	 *           beta in a counter clockwise direction and let the rotating arc
	 *           start at phi and extend width radians in the counter clockwise
	 *           direction. Let the rotating arc be rotating at omega radians
	 *           per unit of time in the counter clockwise direction and be
	 *           located at the location described above at time 0. This
	 *           function will restrict the contents of intervals to only
	 *           contain periods of time during which the two arcs overlap. This
	 *           method only operates and effects periods of time in the
	 *           interval from 0 to 4pi.
	 **/
	private void restrictIntervalByAngle(IntervalList intervals, double alpha, double beta, double omega, double phi,
			double width) {
		if (phi < 0)
	...
	/**
	 * Given the path of a ball and the path of something rotating in a circle,
	 * restricts the possible time periods during which they might collide by
	 * doing some crazy confusing analysis originally concocted by Professor
	 * Ernst. Subclass SimpleGeometry disables these optimizations.
	 *
	 * <p>
	 * <img src="doc-files/restrictSearchInterval-page1.jpg">
	 * <p>
	 * <img src="doc-files/restrictSearchInterval-page2.jpg">
	 * <p>
	 * <img src="doc-files/restrictSearchInterval-page3.jpg">
	 *
	 * @see physics.SimpleGeometry
	 *
	 * @requires: outer_radius > 0, inner_radius < outer_radius, 0 <= phi_1,
	 *            phi_2 < 2pi
	 *
	 * @modifies: intervals
	 *
	 * @effects: returns an IntervalList containing a subset of the times
	 *           contained by <code>intervals</code>. This subset is based on
	 *           when a ball whose initial condition at t=0 is represented by
	 *           <code>ball</code> travelling at a linear velocity of
	 *           <code>velocity</code> might intersect an object rotating at an
	 *           angular velocity of <code>omega</code> about a point specified
	 *           by <code>center</code> which subsumes an arc from
	 *           <code>phi_1</code> to <code>phi_2</code>.
	 **/
	protected IntervalList restrictSearchInterval(IntervalList intervals, double inner_radius, double outer_radius,
			double phi_1, double phi_2, double omega, Vect center, Circle ball, Vect velocity) {
		...
		DoublePair dp = timeUntilCircleCollision(new Circle(center, outer_radius + ball.getRadius()), ball.getCenter(),
				velocity);
		if (!dp.areFinite()) {
		...
		if (inner_radius - ball.getRadius() > 0) {
			dp = timeUntilCircleCollision(new Circle(center, inner_radius - ball.getRadius()), ball.getCenter(),
					velocity);
			if (dp.areFinite()) {
		...
		DoublePair farOuterIntersections = timeUntilCircleCollision(new Circle(center, outer_radius), farRailStartPoint,
				velocity);
		// intersection of the near side of the balls locus of travel with
		// the outer edge of the doughnut swept by the line
		DoublePair nearOuterIntersections = timeUntilCircleCollision(new Circle(center, outer_radius),
				nearRailStartPoint, velocity);
		// intersection of the far side of the balls locus of travel with
		// the inner edge of the doughnut swept by the line
		DoublePair farInnerIntersections = timeUntilCircleCollision(new Circle(center, inner_radius), farRailStartPoint,
				velocity);
		// intersection of the near side of the balls locus of travel with
		// the inner edge of the doughnut swept by the line
		DoublePair nearInnerIntersections = timeUntilCircleCollision(new Circle(center, inner_radius),
				nearRailStartPoint, velocity);

		Vect centerToFarBounds = perpendicularPointWholeLine(
				new LineSegment(farRailStartPoint, farRailStartPoint.plus(velocity)), center);
		Vect centerToNearBounds = perpendicularPointWholeLine(
				new LineSegment(nearRailStartPoint, nearRailStartPoint.plus(velocity)), center);
		...
		if (!farInnerIntersections.areFinite()) {
			...
			// note: the "whiff" (CASE 4) case has already been dealt with
			// above.

			if (!farOuterIntersections.areFinite() && !nearOuterIntersections.areFinite()) {
				// CASE 3
				// this is the case where the size of the ball is larger than
				// the size of the doughnut and thus a hit is imminent.

				// in this case we need only ever consider a single full
				// revolution
				intervals.restrictSubIntervalLength(2 * Math.PI / Math.abs(omega));
			} else {
				if (centerToFarBounds.dot(centerToNearBounds) > 0) {
					// CASE 1 or CASE 2 or CASE 7 or CASE 9
					restrictIntervalByAngle(intervals, nearOuterAngle1, nearOuterAngle2, omega, phi, width, false);
				} else {
					...
					if (nearOuterIntersections.areFinite()) {
						...
						// PAGE 2
						// CASE 8 or CASE 10
						restrictIntervalByAngle(intervals, nearInnerAngle1, nearInnerAngle2, omega, phi, width, true);
					}
				}
			}
		} else {
			...
			// the inner circle
			if (!farInnerIntersections.areFinite() || !farOuterIntersections.areFinite()
					|| !nearInnerIntersections.areFinite() || !nearOuterIntersections.areFinite()) {
				throw new IllegalArgumentException();
			}
			...
			if (centerToFarBounds.dot(centerToNearBounds) > 0) {
				// CASE 11
				restrictIntervalByAngle(intervals, nearOuterAngle1, farInnerAngle1, omega, phi, width, false);
				restrictIntervalByAngle(intervals2, farInnerAngle2, nearOuterAngle2, omega, phi, width, false);
			} else {
				// CASE 12
				restrictIntervalByAngle(intervals, nearInnerAngle1, farInnerAngle1, omega, phi, width, false);
				restrictIntervalByAngle(intervals, farInnerAngle2, nearInnerAngle2, omega, phi, width, false);
			}
	...
	/**
	 * @effects: attempts to return the root of <code>distanceFunction</code> of
	 *           the least value which occurs within <code>intervals</code>. If
	 *           no such root exists, or no such root is found, returns +Inf.
	 **/
	private double searchForCollision(Newton.Function distanceFunction, IntervalList intervals) {
		...
		while (iter.hasNext()) {
			...
			double collisionTime = Newton.findRoot(distanceFunction, interval.start(), interval.end(), t_step);
	 ...
	 **/
	public double timeUntilRotatingWallCollision(LineSegment line, Vect center, double angularVelocity, Circle ball,
			Vect velocity) {
		...
		class RotatingWallDistance implements Newton.Function {
			public Newton.Result evaluate(double t) {
				...
				// now find the intercept of the line segment, and a
				// perpendicular
				...
				double fraction = (((width * (b_x_t - x1_t)) + (height * (b_y_t - y1_t))) / (length_squared));
				...
				double dist_sq = (x_t - b_x_t) * (x_t - b_x_t) + (y_t - b_y_t) * (y_t - b_y_t);
				...
				double length_squared_prime = 2 * width * width_prime + 2 * height * height_prime;

				double fraction_prime = ((length_squared
						* ((width_prime * (b_x_t - x1_t) + width * (b_x_t_prime - x1_t_prime))
								+ (height_prime * (b_y_t - y1_t) + height * (b_y_t_prime - y1_t_prime))))
						- (length_squared_prime * ((width * (b_x_t - x1_t)) + (height * (b_y_t - y1_t)))))
						/ (length_squared * length_squared);

				double x_t_prime = x1_t_prime + fraction * width_prime + fraction_prime * width;
				double y_t_prime = y1_t_prime + fraction * height_prime + fraction_prime * height;

				double f_prime = 2 * (x_t - b_x_t) * (x_t_prime - b_x_t_prime)
		...
		intervals = restrictSearchInterval(intervals, inner_radius, outer_radius, phi_1, phi_2, omega, center, ball,
				velocity);
	 ...
	 **/
	public double timeUntilRotatingCircleCollision(Circle circle, Vect center, double angularVelocity, Circle ball,
			Vect velocity) {
		// we special case the non-rotating case because that scenario
		// has a closed-form solution which is both faster to compute
		// and also is more accurate
		...
		final double r_r2 = (r_x - c_x) * (r_x - c_x) + (r_y - c_y) * (r_y - c_y); // radius
																					// squared
		final double r_r = Math.sqrt(r_r2);
		final double phi = Math.atan2(c_y - r_y, c_x - r_x); // radius of
																// rotation
		...
		IntervalList intervals = new IntervalList(0, maximumForesight);
		intervals = restrictSearchInterval(intervals, inner_radius, outer_radius, phi - arcWidth, phi + arcWidth, omega,
				center, ball, velocity);
	...
	public Vect reflectRotatingCircle(Circle circle, Vect center, double angularVelocity, Circle ball, Vect velocity) {
		return reflectRotatingCircle(circle, center, angularVelocity, ball, velocity, 1.0);
	}
	...
	public Vect reflectRotatingCircle(Circle circle, Vect center, double angularVelocity, Circle ball, Vect velocity,
		...
		if (angularVelocity == 0.0) {
			return reflectCircle(circle.getCenter(), ball.getCenter(), velocity, reflectionCoeff);
		}
		...
		// resulting vector length negative, which is equivalent to a
		// rotation by PI radians.
		Vect myVel = collidePt.times(angularVelocity).rotateBy(Angle.RAD_PI_OVER_TWO);
		// translate into reference frame of the moving circle
		Vect relativeV = velocity.minus(myVel);

		Vect reflectV = reflectCircle(circleCenter, ballCenter, relativeV, reflectionCoeff);
	 ...
	 **/
	public VectPair reflectBalls(Vect center1, double mass1, Vect velocity1, Vect center2, double mass2,
			Vect velocity2) {
		double m1 = mass1;
		double m2 = mass2;
		double m = m1 / m2;
